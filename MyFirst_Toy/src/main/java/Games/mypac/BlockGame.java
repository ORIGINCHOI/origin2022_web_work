...
public class BlockGame {
	...
	static class MyFrame extends JFrame{
		...
		static class Block{
			...
			boolean isHidden = false; //충돌후, 블록 사라짐  메모리제어보단 속성제어로
		}
		
		static class MyPanel extends JPanel{ //드로잉을 위한 캔버스
			public MyPanel() {
		...
		public MyFrame(String title) {
			//JFrame 쪽에 들어있는 생성자 쪽에 연결
			super(title);
			this.setVisible(true);
			this.setSize(CANVAS_WIDTH, CANVAS_HEIGHT);
			this.setLocation(400, 300); // x,y 값 줌
			this.setLayout(new BorderLayout());
			this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
			
			initData(); // 데이터 초기화용 기능
			...
			this.addKeyListener(new KeyAdapter() {
				...
				public void keyPressed(KeyEvent e) { //Key Event
					if(e.getKeyCode() == KeyEvent.VK_LEFT) {
						...
						barXTarget -=20;
						if(bar.x < barXTarget) { //계속 키를 눌렀을 때 방지(너무 많은 값일때)
							barXTarget = bar.x;
						}
					}
